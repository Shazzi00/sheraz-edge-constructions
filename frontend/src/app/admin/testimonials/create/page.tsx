  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && (data.success || data.id)) {
        ...
        setErrorMsg(data.message || 'Failed to create testimonial.');
      }
    } catch (err) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <article className="md:col-span-8 lg:col-span-9 bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200/80 space-y-6">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              <h1 className="text-xl font-bold text-slate-800">Testimonials / Create</h1>
              <Link
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. Director, TechCorp"
                    value={designation}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-600 block">Author Photo</label>
                <input
                  type="file"
                  accept="image/*"
